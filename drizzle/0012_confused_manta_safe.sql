-- Step 1: Create certificate_templates table
...
-- Step 2: Add uuid as nullable first
ALTER TABLE `certificates` ADD `uuid` varchar(64);
ALTER TABLE `certificates` ADD `templateId` int;
ALTER TABLE `certificates` ADD `emailSentAt` timestamp;

-- Step 3: Populate existing rows with a UUID
UPDATE `certificates` SET `uuid` = CONCAT(REPLACE(UUID(), '-', ''), LPAD(id, 8, '0')) WHERE `uuid` IS NULL;

-- Step 4: Make uuid NOT NULL and add unique constraint
ALTER TABLE `certificates` MODIFY `uuid` varchar(64) NOT NULL;
